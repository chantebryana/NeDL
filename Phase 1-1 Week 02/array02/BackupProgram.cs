  ...
  class Program
  {
		...
		static int WriteReadValidate(string output, int min, int max) {
			...
			try {
				...
				Console.WriteLine($"Woops! Error. {e.Message}");
				validate = 0;
			}
		...
		static int GetInt() {
			...
			int convertedInput = Convert.ToInt16(inputString);
			string secondString = Convert.ToString(convertedInput);
			if (inputString == secondString) {
				return convertedInput;
			} else {
				throw new Exception("Woops! Didn't type an acceptable integer");
			}
		}
		...
		static void Main(string[] args)
    {
			...
			int numStudents = 0;
			try {
				question = $"User, how many students? ({low} or greater)";
				numStudents = WriteReadValidate(question, low);
			} catch (Exception e) {
