...
namespace MyApplication
{
  class Program
  {
		...
		static int WriteReadValidate(string output, int min, int max = int.MaxValue) {
			//ask user for input, save results, validate
			while(true) {
				try {
					int input = WriteReadInt(output);
					if(ValidateInput(input, min, max))
						return input; // the only way out
					Console.WriteLine("Woops, invalid input");
				} catch (Exception e) {
					Console.WriteLine($"Woops! Conversion Error: '{e.Message}'");
				}
			}
		}
    ...
    static bool ValidateInput(int userInput, int low, int high) {
        //if incorrect   
